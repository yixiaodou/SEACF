...
class PreTrainedBertModel(nn.Module):
    ...
    def from_pretrained(cls,
        ...
        if state_dict is None:
            ...
            weights_path = os.path.join(serialization_dir, 'model.safetensors')
...
class CaptionTextAlignment(nn.Module):
    ...
    def forward(self, Hc, Hs):
        attn_out, _ = self.mca(Hc, Hs, Hs)

        H_hat = self.ln1(Hc + attn_out)
        H_tilde = self.ln2(self.ffn(H_hat) + H_hat)

        sim = F.cosine_similarity(
            Hs.mean(dim=1, keepdim=True),
            H_tilde,
            dim=-1
        ).unsqueeze(-1)
...
class GatedFusion(nn.Module):
    def __init__(self, img_dim=2048, hidden_size=768):
        super().__init__()
        ...
        # gate 控制比例
        self.gate = nn.Linear(img_dim + hidden_size, hidden_size)

    def forward(self, Hi, HSC):
        """
        Hi:  (B, 2048)
        HSC: (B, 768)
        """
        Hi_proj = self.img_proj(Hi)   # → (B, 768)

        g = torch.sigmoid(
            self.gate(torch.cat([Hi, HSC], dim=-1))
        )

        H_fused = g * HSC + (1 - g) * Hi_proj
        return H_fused
...
class GraphConvolution(nn.Module):
    ...
    def forward(self, text, adj,mask=None):
        # text: [batch, seq_len, in_features]
        # adj: [batch, seq_len, seq_len]
        hidden = torch.matmul(text, self.weight)
        # denom = torch.sum(adj, dim=2, keepdim=True) + 1
        denom = torch.sum(adj, dim=2, keepdim=True).clamp(min=1)
...
class SyntaxGCN(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        for layer in self.layers:
            ...
            x = self.dropout(x)
        return x
class SyntaxSemanticFusion(nn.Module):
    def __init__(self, hidden_size, num_heads=8):
        super(SyntaxSemanticFusion, self).__init__()
        self.attention = nn.MultiheadAttention(embed_dim=hidden_size, num_heads=num_heads, batch_first=True)
        self.layer_norm = nn.LayerNorm(hidden_size)
        self.dropout = nn.Dropout(0.1)

    def forward(self, Hs, Hsys, key_padding_mask=None):
        # Q = Hs (BERT Context), K = V = Hsys (Syntax GCN)
        # key_padding_mask: [Batch, Seq] (1 for pad, 0 for keep usually, but Torch MHA needs careful checking. 
        # Here we usually pass attention masks. Note: PyTorch MHA expects key_padding_mask as True for positions to IGNORE)
        
        # Note: Bert's mask is 1 for keep, 0 for mask. 
        # PyTorch MHA key_padding_mask expects: True for ignore, False for keep.
        if key_padding_mask is not None:
            key_padding_mask = (key_padding_mask == 0)

        attn_output, _ = self.attention(query=Hs, key=Hsys, value=Hsys, key_padding_mask=key_padding_mask)
        output = self.layer_norm(Hs + self.dropout(attn_output)) # Residual connection
        return output
class CaptionRegionFusion(nn.Module):
    def __init__(self, dim=768):
        super().__init__()
        self.img_proj = nn.Linear(2048, dim)
        self.cap_proj = nn.Linear(768, dim)
        self.attn = nn.MultiheadAttention(dim, num_heads=8, batch_first=True)
        self.ln = nn.LayerNorm(dim)

    def forward(self, regions, caption_tokens, cap_mask=None):
        R = self.img_proj(regions)      # [B, 36, 768]
        C = self.cap_proj(caption_tokens)  # [B, Lc, 768]
        ...
        Cg = self.ln(C + attn_out)      # caption grounded

        # 反向再让 region 被 caption 约束
        attn_r, _ = self.attn(
            query=R,
            key=Cg,
            value=Cg
        )
        Rg = self.ln(R + attn_r)   # grounded regions

        return Cg, Rg


class SyntaxAwareSelfAttention(nn.Module):
    def __init__(self, hidden_size, num_heads=8, lambda_init=1.0):
        super().__init__()
        ...
        # 语法强度（可学习）
        self.lambda_dep = nn.Parameter(
            torch.tensor(lambda_init, dtype=torch.float)
        )
        self.dropout = nn.Dropout(0.1)

    def forward(self, x, dep_adj, key_padding_mask=None):
        """
        x: [B, L, D]
        dep_adj: [B, L, L]  (0/1 or weighted)
        key_padding_mask: [B, L]  (1=valid, 0=pad)
        """
        B, L, D = x.size()
        ...
        # ===== syntax bias =====
        # dep_adj: [B, 1, L, L]
        dep_bias = self.lambda_dep * dep_adj.unsqueeze(1)
        attn_scores = attn_scores + dep_bias
...
# class HierarchicalTransformer_TopN_Attention_v2(nn.Module):
#     def __init__(self,
#                  bert_path,
#                  num_labels=3,
#                  embedding=None,
#                  TAT_num=1,
#                  MFT_num=1):
#         # 强制修改 Dropout 为 0.4 (原默认值为 0.1) # 这是缓解过拟合最直接有效的手段
#         # config.hidden_dropout_prob = 0.2           # 全连接层丢弃率
#         # config.attention_probs_dropout_prob = 0.2  # Attention矩阵丢弃率
#         super().__init__()  
...
#             bert_path,
#             config=config,)
#         self.target_bert = AutoModel.from_pretrained(
#             bert_path,
#             config=config,
#         )
#         self.caption_bert = self.bert
#         self.syntax_attn = SyntaxAwareSelfAttention(
#             hidden_size=config.hidden_size,
#             num_heads=8,
#             lambda_init=1.0
#         )
#         self.Wr = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
#         self.mse = torch.nn.MSELoss()
#         self.left_TAT = BertCrossEncoder(config, layer_num=TAT_num)
#         self.right_TAT = BertCrossEncoder(config, layer_num=TAT_num)
#         self.text_img_interaction = BertCrossEncoder(config, layer_num=1)
...
#         self.pooler = BertPooler(config)
#         # caption pooling  # gate controller
#         self.hsc_pooler = AttentionPooling(config.hidden_size)
#         self.caption_region_fusion = CaptionRegionFusion(
...
#         self.text_fusion = SyntaxSemanticFusion(config.hidden_size)

#     def forward(self, context_ids, context_mask, context_segment_ids,
#                 target_ids, target_mask, target_segment_ids,
#                 caption_ids, caption_mask, caption_segment_ids,
#                 img_feature,img_mask, img_sematic_ids, combine_mask, dep_adj_matrix,copy_flag):
#         if copy_flag:
#             print(
...
#         )
#         context_output = outputs.last_hidden_state
#         Hs = context_output 
#         Hsyn = self.syntax_attn(
#             Hs,
#             dep_adj_matrix,
#             key_padding_mask=context_mask
#         )

#         Hs_sys = self.text_fusion(
#             Hs,
#             Hsyn,
#             key_padding_mask=context_mask
#         )
#         caption =   self.caption_bert(
#             input_ids=caption_ids,
#             attention_mask=caption_mask)
#         caption_output = caption.last_hidden_state

#         Cg, Rg = self.caption_region_fusion(
#             img_feature,
#             caption_output,
#             caption_mask
#         )
#         caption_visual = Cg.mean(dim=1)    # [B, 768] 图像感知的 caption
     
#         Hi_global = img_feature.mean(dim=1) #  图像全局表示:36个区域压成一个整体图像表示
#         # Hi_global = Rg.mean(dim=1)
#         HV = self.gated_fusion(
#             Hi_global,
#             caption_visual
#         )   # [B,768]
      
#         v2t_img_feature = Rg + HV.unsqueeze(1)
        
#         target = self.target_bert(
#             input_ids=target_ids,
#             attention_mask=target_mask  )
#         target_output = target.last_hidden_state
#         '''
#         ----------------------------------------------------------------------------------------------
#         '''
...
#         ----------------------------------------------------------------------------------------------
#         '''
#         target_aware_context = self.left_TAT(target_output, Hs_sys,
#                                              extended_context_mask)[-1]
#         target_aware_image = self.right_TAT(target_output, v2t_img_feature,
#                                             extended_img_mask)[-1]

#         reconstruct_from_context = self.Wr(target_aware_context)
#         reconstruct_from_image = self.Wr(target_aware_image)
#         rc = reconstruct_from_context.mean(dim=1)   # [B, H]
#         ri = reconstruct_from_image.mean(dim=1)     # [B, H]
#         co = Hs_sys.mean(dim=1)             # [B, H]
#         r_loss = (self.mse(rc, co) + self.mse(ri, co)) / 2

#         # 3. 计算 HSI (视觉增强的文本特征)
#         # Query: Hs_sys (文本), Key/Value: v2t_img_feature (图像)
#         # Mask: 使用 extended_img_mask，因为 KV 是图像
#         # HSI shape: [B, Text_Len, 768] (与 Hs_sys 长度一致)
#         # HSI = self.text_img_interaction(Hs_sys, v2t_img_feature, extended_img_mask)[-1]
#         HSI = self.text_img_interaction(Hs, v2t_img_feature, extended_img_mask)[-1]

#         # left_MFT: Query=target_aware_context, Key=Value=HSI
#         multimodality_fused_context_branch = self.left_MFT(
#             target_aware_context, 
#             HSI, 
#             extended_target_mask, # 关键修改：KV是HSI(文本长度)，用context_mask
#             extended_context_mask  # attention_mask
#         )
#         # right_MFT: Query=target_aware_image, Key=Value=HSI
#         # 逻辑：让 Target-Aware Image 再次去查阅 "视觉增强后的文本上下文"
#         multimodality_fused_image_branch = self.right_MFT(
#             target_aware_image, 
#             HSI, 
#             extended_target_mask, # 关键修改：KV是HSI(文本长度)，用context_mask
#             extended_context_mask
#         )
#         # 5. Final Classification
#         cls_multimodality_fused = multimodality_fused_context_branch[:, 0].unsqueeze(1)
#         combine_representation = torch.cat(
#             (cls_multimodality_fused, multimodality_fused_image_branch),
#             dim=1)
...
class HierarchicalTransformer_TopN_Attention_v2(nn.Module):
    def __init__(self,
        ...
        # config.attention_probs_dropout_prob = 0.2  # Attention矩阵丢弃率
        self.num_labels = num_labels
        # self.use_hsyn = use_hsyn  # 保存Hsyn开关
        # self.use_caption = use_caption  # 保存caption开关
        ...
        self.text_fusion = SyntaxSemanticFusion(config.hidden_size)
        self.img_proj = nn.Linear(2048, config.hidden_size)  
    def forward(self, context_ids, context_mask, context_segment_ids,
                target_ids, target_mask, target_segment_ids,
                caption_ids, caption_mask, caption_segment_ids,
                img_feature,img_mask, img_sematic_ids, combine_mask, dep_adj_matrix,copy_flag,use_hsyn=True,
    use_caption=True):
        if copy_flag:
            print(
                "*************************copy bert's weights to s2_bert******************"
        ...
        if use_hsyn:
            ...
            # 无Hsyn：直接使用原始context_output，跳过句法注意力和融合
            Hs_sys = Hs 
        ...
        # Hi_global = Rg.mean(dim=1)
            ...
            Hi_global = img_feature_proj.mean(dim=1)
            HV = Hi_global
            v2t_img_feature = img_feature_proj + HV.unsqueeze(1)  
        ...
        tsne_feature = combine_output[:, 0, :]   
      
        

        # ========== 6. 返回值修改：新增feat_dict ==========
        return logits, r_loss, tsne_feature
