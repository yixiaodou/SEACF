...
from my_bert.mm_modeling_out_no_self import HierarchicalTransformer_TopN_Attention_v2
...
class MultimodalFeatures(object):
    def __init__(self, context_ids, context_mask, context_segment_ids,
                 ...
                 img_features, img_mask, img_sematic_ids, combine_mask, dep_adj_matrix,label_id):
        ...
        self.caption_segment_ids = caption_segment_ids
...
def dependency_adj_matrix_wordlevel(text, nlp, max_len):
    """
    词级依存邻接矩阵（干净版）
    - 节点 = spaCy 单词（word-level）
    - 去掉 rt / @username / 非字母
    - padding 行补自环，防止 0-row

    return:
        adj   : [max_len, max_len] float32
        words : 实际使用的词（debug 用）
    """
    doc = nlp(text)

    # ===== 1. 构建干净词表 =====
    words = []
    idx_map = {}   # spaCy token.i -> word index
    ...
    # ===== 3. 构建依存边（仅限合法词）=====
    for token in doc:
        if token.i not in idx_map:
            continue
        if token.head.i not in idx_map:
            continue

        i = idx_map[token.i]
        j = idx_map[token.head.i]

        if i < max_len and j < max_len:
            adj[i, j] = 1.0
            adj[j, i] = 1.0   # 无向图（GCN 稳定）
...
def convert_mmexamples_to_mmfeatures(examples, label_list, args, tokenizer,
                                     wordbag, img_features_reader, caption_dict, mode):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # RoBERTa没有token_type_ids，全部设0即可
        segment_ids = [0] * args.max_seq_length

        # ===== 句法邻接矩阵（仍然基于原始文本）=====
        adj_matrix, word_nodes = dependency_adj_matrix_wordlevel(
            example.text_a,
            nlp,
            args.max_seq_length
        )
        ...
        if ex_index < 1:
            ...
            print("Adj row sum:", adj_matrix.sum(axis=1))
        ...
        target_input_mask = target_encoded["attention_mask"]
        target_segment_ids = [0] * args.max_target_length

        # ===== caption =====
        caption_text = caption_dict.get(example.img_id, "")
        ...
        caption_ids = caption_encoded["input_ids"]
        caption_mask = caption_encoded["attention_mask"]
        caption_segment_ids = [0] * args.max_seq_length
        ...
        for text in img_semantics:
            ...
            now_id_list = []
            if len(words) > 3:
                attr = wordbag.get(words[0], 0)
            else:
                attr = 0
            for word in words[-3:]:
                word_id = wordbag.get(word, 0)
...
def get_DataLoader(args,
    ...
    features = convert_mmexamples_to_mmfeatures(examples, label_list, args,
                                                tokenizer, wordbag,
                                                img_features_reader, caption_dict, mode)
    ...
    all_target_segment = torch.tensor([f.target_segment_ids for f in features],
                                      dtype=torch.long)
    all_caption_ids = torch.tensor([f.caption_ids for f in features], dtype=torch.long)
    all_caption_mask = torch.tensor([f.caption_mask for f in features], dtype=torch.long)
    all_caption_segment = torch.tensor([f.caption_segment_ids for f in features], dtype=torch.long)
    ...
    all_combine_mask = torch.tensor([f.combine_mask for f in features],
                                    dtype=torch.long)
    all_dep_adj = torch.tensor([f.dep_adj_matrix for f in features], dtype=torch.float)
    all_label_ids = torch.tensor([f.label_id for f in features],
                                 dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                            all_target_ids,all_target_mask,all_target_segment,
                            all_caption_ids,all_caption_mask,all_caption_segment,  
                            all_img_features, all_img_mask,
                            all_img_semantics, all_combine_mask, all_dep_adj,all_label_ids)
    sampler = None
    dataloader = None
    if mode == 'Train':
...
def main():
    ...
    parser.add_argument('--wordbag_path',
                        default="/home/haust/Code/HIMT/data/mywordbag.txt")
    parser.add_argument('--embedding_path',
                        default="/home/haust/Code/HIMT/data/myembedding.txt")
    ...
    print("type(tokenizer)是",type(tokenizer))

    def load_caption_dict(task_name):
        if task_name == 'twitter2015':
            path = 'data/caption/twitter2015_images.json'
        else:
            path = 'data/caption/twitter2017_images.json'
        with open(path, 'r') as f:
            return json.load(f)
    caption_dict = load_caption_dict(args.task_name)
    ...
    model = HierarchicalTransformer_TopN_Attention_v2(
    bert_path=args.bert_model,
    num_labels=num_labels,
    embedding=embedding
    )
    print("model.bert.config.hidden_size",model.bert.config.hidden_size)
    print("type(model.bert)是",type(model.bert))
    print("\n\n\n\n\n")
    param_sum = sum(p.numel() for p in model.parameters())
    print("param_sum是:", param_sum)
    ...
    if args.do_train:
        train_dataloader = get_DataLoader(args, train_examples, label_list,
                                          tokenizer, wordbag,caption_dict=caption_dict,mode='Train')
        #'''
        ...
        for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(
                ...
                input_ids, input_mask, segment_ids, target_ids, target_mask, target_segment, caption_ids, caption_mask, caption_segment_ids,img_features, img_mask, img_semantic_ids, combine_mask, dep_adj_matrix,label_ids = batch
                ...
                loss_fct = LabelSmoothingLoss(classes=3, smoothing=0.1) # <-- 用这个

                loss = loss_fct(logits.view(-1, 3),
                                label_ids.view(-1)) + args.r_loss * r_loss
                with torch.no_grad():
                    preds = torch.argmax(logits, dim=-1)
                    tr_correct += (preds == label_ids).sum().item()
                    tr_total_samples += label_ids.size(0)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    optimizer.zero_grad()
                    global_step += 1
            # 【新增】计算本 Epoch 的平均训练 Loss 和 Accuracy
            epoch_train_loss = tr_loss / nb_tr_steps
            epoch_train_acc = tr_correct / tr_total_samples
            logger.info(f"==> Epoch {train_idx} Training Result: Loss={epoch_train_loss:.4f}, Accuracy={epoch_train_acc:.4f}")
            ...
            for batch in tqdm(eval_dataloader, desc="Evaluating"):
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, target_ids, target_mask, target_segment, \
                caption_ids, caption_mask, caption_segment_ids, \
                img_features, img_mask, img_semantic_ids, combine_mask, dep_adj_matrix,label_ids = batch
                ...
                with torch.no_grad():
                    ...
                    ce_loss = loss_fct(logits.view(-1, 3), label_ids.view(-1))
                    loss = ce_loss + args.r_loss * r_loss
                ...
                true_label_list.append(label_ids.cpu().numpy())
                pred_label_list.append(logits.cpu().numpy())
            ...
            # 2. 计算每一类的 F1 分数
            # 先获取预测的类别索引
            pred_labels_indices = np.argmax(pred_outputs, axis=-1)
            
            # average=None 表示返回每个类别的指标
            # labels=[0, 1, 2] 确保即使某个类别从未被预测到，也能返回3个数值（不够的补0）
            _, _, f_per_class, _ = precision_recall_fscore_support(
                true_label, 
                pred_labels_indices, 
                average=None, 
                labels=[0, 1, 2]
            ...
            f1_class_0 = f_per_class[0] # 负面 (通常 label 0 是 Negative)
            f1_class_1 = f_per_class[1] # 中性 (通常 label 1 是 Neutral)
            f1_class_2 = f_per_class[2] # 正面 (通常 label 2 是 Positive)
            result = {
                'eval_loss': eval_loss,
                'eval_accuracy': eval_accuracy,
                'f_score': F_score,
                'train_loss': epoch_train_loss, # 记录训练Loss
                'train_accuracy': epoch_train_acc, # 记录训练Accuracy
            ...
            if F_score >= max_f1:
                ...
                max_f1 = F_score
        log_file_path = os.path.join(args.output_dir, "training_logs.csv")
        with open(log_file_path, 'w') as f:
            writer = csv.writer(f)
            writer.writerow(['Epoch', 'Train_Loss', 'Train_Acc', 'Dev_Loss', 'Dev_Acc', 'Dev_F1','F1_Class_0', 'F1_Class_1', 'F1_Class_2'])
    ...
    if args.do_eval and (args.local_rank == -1
        ...
        # average=None 表示不聚合，返回每个类别的独立分数
        _, _, f1_per_class, _ = precision_recall_fscore_support(
            true_label, pred_indices, average=None
        )
        result = {
            'eval_loss': eval_loss,
            'eval_accuracy': eval_accuracy,
            ...
            'recall': recall,
            'f_score': F_score,
            'f1_class_0': f1_per_class[0], # 0类 F1
            'f1_class_1': f1_per_class[1], # 1类 F1
            'f1_class_2': f1_per_class[2], # 2类 F1
            'global_step': global_step,
            "lr": args.learning_rate
